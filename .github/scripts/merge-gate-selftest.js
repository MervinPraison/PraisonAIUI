...
assert('ci-only label not exempt mixed changes', mg.sensitivePathReasons(
  [{ filename: '.github/workflows/foo.yml' }, { filename: 'src/praisonaiui/foo.py' }],
  [mg.WORKFLOW_ONLY_LABEL]
).length === 1);
...
// Tests heuristic
assert('product code without tests', mg.missingTestsReason([{ filename: 'src/praisonaiui/a/b.py', additions: 3 }]) !== null);
assert('product code with tests ok', mg.missingTestsReason([
  { filename: 'src/praisonaiui/a/b.py', additions: 3 },
...
assert('internal PR link accepted', mg.isInternalPullRequestLink(
  { base: { repo: { full_name: 'MervinPraison/PraisonAIUI' } } },
  'MervinPraison',
  'PraisonAIUI'
));
assert('fork sync PR link rejected', !mg.isInternalPullRequestLink(
  { number: 21, base: { repo: { full_name: 'Milkmange/PraisonAIUI' } } },
  'MervinPraison',
  'PraisonAIUI'
));
