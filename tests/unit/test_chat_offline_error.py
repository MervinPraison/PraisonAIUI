...
error feedback. These source-parsing tests assert the fixes:
  ...
  RC-2  selectPage shows a shell error (.db-error / role=alert via
        showDashboardInitError) when #db-main-content is missing instead
...
class TestSelectPageShellError:
    ...
    def test_select_page_calls_shell_error_on_missing_main(self):
        ...
        assert "showDashboardInitError(" in region
