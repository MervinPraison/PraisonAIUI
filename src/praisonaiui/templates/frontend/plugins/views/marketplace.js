...
export async function render(container) {
  ...
  function cardHtml(p) {
    ...
    return `
      <div class="db-card" data-plugin="${p.id}" style="padding:16px 18px;display:flex;flex-direction:column;gap:10px;cursor:pointer">
        <div style="display:flex;align-items:center;gap:10px">
          <span style="font-size:22px">${catIcon(p.category)}</span>
          <div style="flex:1;min-width:0">
            <div style="font-size:14px;font-weight:600">${p.name || p.id}</div>
            <div style="font-size:11px;color:var(--db-text-dim)">${p.category || 'plugin'} · v${p.version || '1.0.0'}</div>
          </div>
        ...
        <div style="font-size:12px;color:var(--db-text-dim);line-height:1.5;min-height:34px">${p.description || ''}</div>
        <div style="display:flex;gap:8px">
          <button type="button" class="mkt-open" data-plugin="${p.id}" style="flex:1;padding:6px 10px;border:1px solid var(--db-border);background:transparent;color:var(--db-text);border-radius:7px;cursor:pointer;font-size:12px">Details</button>
          ${installed
            ? `<button type="button" class="mkt-uninstall" data-plugin="${p.id}" style="flex:1;padding:6px 10px;border:1px solid rgba(239,68,68,.35);background:rgba(239,68,68,.08);color:#f87171;border-radius:7px;cursor:pointer;font-size:12px">Uninstall</button>`
            : `<button type="button" class="mkt-install" data-plugin="${p.id}" style="flex:1;padding:6px 10px;border:1px solid var(--db-accent);background:var(--db-accent);color:#fff;border-radius:7px;cursor:pointer;font-size:12px">Install</button>`}
        </div>
      </div>`;
  }
      ...
      <div class="mkt-drawer-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:120;display:flex;justify-content:flex-end">
        <div style="width:min(480px,100%);height:100%;background:var(--db-sidebar-bg);border-left:1px solid var(--db-border);overflow-y:auto;padding:24px;box-sizing:border-box">
          <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:16px">
            ...
            <div style="flex:1">
              <div style="font-size:18px;font-weight:700">${p.name || p.id}</div>
              <div style="font-size:12px;color:var(--db-text-dim)">${p.category || 'plugin'} · v${p.version || '1.0.0'}</div>
            </div>
          ...
          <div style="font-size:13px;line-height:1.6;color:var(--db-text-dim);margin-bottom:20px">${p.description || 'No description provided.'}</div>
          ...
          <div style="display:flex;flex-direction:column;gap:6px;margin-bottom:24px">
            ${perms.length
              ? perms.map((perm) => `<div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid var(--db-border);border-radius:8px;font-size:12px"><span>🔐</span><code style="font-size:12px">${perm}</code></div>`).join('')
              : '<div style="font-size:12px;color:var(--db-text-dim)">No special permissions requested.</div>'}
          </div>
          ${installed
            ? `<button type="button" class="mkt-uninstall" data-plugin="${p.id}" style="width:100%;padding:11px;border:1px solid rgba(239,68,68,.35);background:rgba(239,68,68,.08);color:#f87171;border-radius:9px;cursor:pointer;font-size:14px;font-weight:600">Uninstall</button>`
            : `<button type="button" class="mkt-install" data-plugin="${p.id}" style="width:100%;padding:11px;border:1px solid var(--db-accent);background:var(--db-accent);color:#fff;border-radius:9px;cursor:pointer;font-size:14px;font-weight:600">Install ${perms.length ? '(' + perms.length + ' permissions)' : ''}</button>`}
        </div>
      </div>`;
  }
