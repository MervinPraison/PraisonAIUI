...
export async function render(container) {
  ...
  const platformMeta = {
    discord:    { icon: '🎮', color: '#5865F2', fields: ['Bot Token', 'Server ID', 'Channel ID'] },
    slack:      { icon: '💬', color: '#4A154B', fields: ['Bot Token', 'Signing Secret', 'Channel'] },
    telegram:   { icon: '✈️', color: '#229ED9', fields: ['Bot Token', 'Chat ID', 'Webhook URL'] },
  ...
  container.querySelector('#ch-add')?.addEventListener('click', () => {
    ...
    function updatePlatformFields() {
      ...
      fieldsDiv.innerHTML = pm.fields.map((f, i) => `
        <label style="display:block;margin-bottom:10px">
          ...
          <input class="chf-field" data-field="${f.toLowerCase().replace(/\s+/g, '_')}" type="${f.toLowerCase().includes('token') || f.toLowerCase().includes('secret') || f.toLowerCase().includes('key') ? 'password' : 'text'}" placeholder="${f}" style="width:100%;padding:8px 12px;background:var(--db-card-bg);border:1px solid var(--db-border);border-radius:6px;color:var(--db-text);font-size:14px;box-sizing:border-box">
        </label>
      `).join('');
    ...
    m.querySelector('#chf-save').addEventListener('click', async () => {
      const body = { platform: m.querySelector('#chf-platform').value, name: m.querySelector('#chf-name').value };
      m.querySelectorAll('.chf-field').forEach(f => { body[f.dataset.field] = f.value; });
