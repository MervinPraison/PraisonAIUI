...
class GuardrailProtocol(ABC):
    ...
    @abstractmethod
    def get_violations(
        self, *, limit: int = 50, level: Optional[str] = None
    ) -> List[Dict[str, Any]]:
    ...
    @abstractmethod
    def log_violation(
        self, agent_id: str, guardrail: str, message: str, level: str = "WARNING"
    ) -> None:
...
class SimpleGuardrailManager(GuardrailProtocol):
    ...
    def __init__(self) -> None:
        ...
        self._registry: Dict[str, Dict[str, Any]] = (
            saved.get("registry", {}) if isinstance(saved, dict) else {}
        )
                ...
                if gw is not None:
                    for aid in gw.list_agents():
                        ...
                        if grs and isinstance(grs, list):
                            ag.guardrails = [
                                g for g in grs if getattr(g, "_aiui_id", None) != guardrail_id
                            ]
            except (ImportError, Exception):
                pass
        return deleted
        ...
        return [
            {
                ...
                "commands": {
                    ...
                    "status": {
                        "help": "Show guardrails system status",
                        "handler": self._cli_status,
                    },
                    "violations": {
                        "help": "Show recent guardrail violations",
                        "handler": self._cli_violations,
                    },
                },
            }
        ]
...
def log_violation(agent_id: str, guardrail: str, message: str, level: str = "WARNING") -> None:
...
async def check_guardrails(
    text: str, agent_name: str = "", direction: str = "input"
